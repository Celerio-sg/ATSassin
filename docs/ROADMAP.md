...
- **SQLite migrations are fixed (#181 closed):** schema changes use an ordered, append-only `PRAGMA user_version` registry shared by pipeline and feedback entry points, with long-lived operations holding a database transaction across their version guard and SQL. After acquiring the migration write lock, a pre-existing database is backed up with SQLite's consistent backup API before the pending sequence runs in that transaction; failed or newer schemas are refused without modifying user history. This is the substrate for #142 and #182.
