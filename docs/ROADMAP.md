...
- **SQLite migrations are fixed (#181 closed):** schema changes use an ordered, append-only `PRAGMA user_version` registry shared by pipeline and feedback entry points, with every operation holding a database transaction across its version guard and SQL. After acquiring the migration write lock, backup eligibility is determined from the locked state and a pre-existing database is backed up with SQLite's consistent backup API and source permissions before the pending sequence runs in that transaction; failed or newer schemas are refused without modifying user history. This is the substrate for #142 and #182.
- **Canonical content-addressed job IDs are fixed (#142 closed):** posting URLs are canonicalised and hashed, explicitly identified search-page leads use a normalised company/title/location tuple, and file imports use a content hash ([ADR-001](DECISIONS.md#adr-001--job-identity-is-content-addressed-never-random)). Schema v3 preserves and retargets all dependent history while collapsing legacy duplicates. Repeat upserts are reported as updates, so the CLI and daemon do not re-rank or re-evaluate an existing posting.
- **The shared egress boundary is fixed (#143, #81, and #71 closed):** Lightning AI accepts only an opaque payload containing the exact bytes checked immediately before upload; validation fails closed and no flagged copy is retained. Representative SG, UK, India, EU, and US address/phone/ID fixtures plus false-positive controls are covered. Every LLM route now uses the same egress module for explicit untrusted-data markers, nested-instruction rejection, and context-derived prompt budgets; raw prompt transport is not public. Deterministic PII matching remains bounded coverage, not universal entity recognition.
