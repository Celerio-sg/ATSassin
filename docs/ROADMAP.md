...
- **PII egress containment and deterministic detector breadth are fixed (#143 and #81 closed):** Lightning AI accepts only an opaque payload containing the exact bytes checked immediately before upload; validation fails closed and no flagged copy is retained. Representative SG, UK, India, EU, and US address/phone/ID fixtures plus false-positive controls are covered. This remains deterministic pattern matching, not universal entity recognition.
