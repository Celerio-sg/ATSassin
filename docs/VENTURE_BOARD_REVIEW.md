...
2. **Historical finding — fixed by #142 (closed): job identity was random.** All live scan paths now use content-addressed identities with explicit posting/search-page provenance. SQLite schema v3 transactionally re-keyed legacy jobs and retargeted every evaluation, pipeline, application, and feedback row before duplicate collapse. Canonical upserts distinguish inserts from refreshes, so repeat daemon ticks do not schedule existing jobs for evaluation. The 2026-07-29 trial observation—8 of the top 20 recommendations were duplicates—is retained as historical evidence for the defect.
